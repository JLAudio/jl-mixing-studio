...
export function DeliveryOptionsDialog({ request, projectName, onChange, onPreview, onClose }: {
  ...
  return <div className="dialog-backdrop" onKeyDown={(event) => { if (event.key === "Escape") onClose(); }}><section className="client-dialog" role="dialog" aria-modal="true" aria-labelledby="delivery-options-title"><p className="kicker">{productCopy.delivery.guided}</p><h2 id="delivery-options-title">{request.replacementMode === "default" ? productCopy.delivery.createPackage : productCopy.delivery.rebuildPackage}</h2><p className="dialog-intro">Choose how to package <strong>{projectName}</strong>. You’ll review the exact files and changes before anything happens.</p>
    ...
    <div className="dialog-actions"><button type="button" className="secondary" onClick={onClose}>{productCopy.delivery.cancel}</button><button type="button" onClick={onPreview}>{productCopy.delivery.previewPackage}</button></div>
  </section></div>;
}
    ...
    <div className="dialog-backdrop" onKeyDown={(event) => { if (event.key === "Escape" && !pending) onClose(); }}>
      <section className="client-dialog" role="dialog" aria-modal="true" aria-labelledby="delivery-dialog-title">
        ...
        {state.status === "uncertain" ? <>
          ...
          <div className="dialog-actions"><button type="button" onClick={onClose}>{productCopy.delivery.close}</button></div>
        </> : <>
          ...
          <dl className="confirmation-list">
            ...
            <div><dt>{productCopy.delivery.currentRevision}</dt><dd>Revision {state.preview.currentRevision}</dd></div>
          ...
          <div className="dialog-actions"><button type="button" className="secondary" onClick={onClose} disabled={pending}>{productCopy.delivery.cancel}</button><button ref={confirmButton} type="button" onClick={onConfirm} disabled={pending || (state.preview.replacementMode === "clean" && cleanConfirmation !== cleanPhrase)}>{pending ? productCopy.delivery.creating : state.preview.replacementMode === "clean" ? productCopy.delivery.cleanAndRebuild : state.preview.replacementMode === "overwrite" ? productCopy.delivery.rebuildDelivery : productCopy.delivery.createDelivery}</button></div>
        </>}
      </section>
    </div>
  );
      ...
      <section className="client-dialog" role="dialog" aria-modal="true" aria-labelledby="revision-dialog-title">
        ...
        {(state.status === "editing" || state.status === "preflighting") && (
          <form onSubmit={onPreflight} noValidate>
            ...
            <div className="dialog-actions"><button type="button" className="secondary" onClick={onClose} disabled={pending}>{productCopy.delivery.cancel}</button><button type="submit" disabled={pending} aria-busy={pending}>{pending ? "Checking…" : "Review revision"}</button></div>
          </form>
        )}
        {(state.status === "confirming" || state.status === "creating") && (
          <div>
            ...
            <dl className="confirmation-list">
              ...
              <div><dt>{productCopy.delivery.currentRevision}</dt><dd>Revision {project.currentRevision}</dd></div>
              <div><dt>New revision</dt><dd>Revision {state.preview.number}</dd></div>
              <div><dt>Description</dt><dd>{state.preview.description}</dd></div>
            </dl>
            <div className="dialog-actions"><button type="button" className="secondary" onClick={onClose} disabled={pending}>{productCopy.delivery.cancel}</button><button type="button" className="secondary" onClick={onBack} disabled={pending}>Back</button><button ref={confirmButton} type="button" onClick={onConfirm} disabled={pending} aria-busy={pending}>{pending ? "Creating…" : "Create revision"}</button></div>
          </div>
        )}
        {state.status === "uncertain" && (
          <div><div className="form-error" role="alert">{state.message}</div><p className="dialog-intro">Do not create another revision automatically. Close this message, refresh Revisions, and verify the result before trying again.</p><div className="dialog-actions"><button type="button" onClick={onClose}>{productCopy.delivery.close}</button></div></div>
        )}
      </section>
    </div>
  );
      ...
      <section className="client-dialog" role="dialog" aria-modal="true" aria-labelledby="approval-dialog-title">
        ...
        {(state.status === "editing" || state.status === "preflighting") && (
          <form onSubmit={onPreflight} noValidate>
            ...
            <div className="dialog-actions"><button type="button" className="secondary" onClick={onClose} disabled={pending}>{productCopy.delivery.cancel}</button><button type="submit" disabled={pending} aria-busy={pending}>{pending ? "Checking…" : "Review approval"}</button></div>
          </form>
        )}
        {(state.status === "confirming" || state.status === "approving") && (
          <div>
    ...
    <div className="dialog-backdrop" onKeyDown={(event) => { if (event.key === "Escape" && !pending) onClose(); }}>
      <section className="client-dialog intake-dialog" role="dialog" aria-modal="true" aria-labelledby="intake-dialog-title">
        ...
        {state.status === "uncertain" ? <><div className="form-error" role="alert">{state.message}</div><p className="dialog-intro">Do not run intake validation again automatically. Close this message, refresh Intake, and verify the report before trying again.</p><div className="dialog-actions"><button type="button" onClick={onClose}>{productCopy.delivery.close}</button></div></> : <>
          ...
          <div className="dialog-actions"><button type="button" className="secondary" onClick={onClose} disabled={pending}>{productCopy.delivery.cancel}</button><button ref={confirmButton} type="button" onClick={onConfirm} disabled={pending} aria-busy={pending}>{pending ? "Updating report…" : "Update intake report"}</button></div>
        </>}
      </section>
    </div>
  );
      ...
      >
        ...
        {state.status === "uncertain" && (
          <div>
            ...
            <div className="dialog-actions">
              <button type="button" onClick={onClose}>{productCopy.delivery.close}</button>
            </div>
          </div>
        )}
      </section>
      ...
      >
        ...
        {editing && (
          <form onSubmit={onPreflight} noValidate>
            ...
            <div className="dialog-actions">
              <button type="button" className="secondary" onClick={onClose} disabled={pending}>{productCopy.delivery.cancel}</button>
