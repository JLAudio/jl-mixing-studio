...
path = Path("src-tauri/src/workspace.rs")
source = path.read_text()
start = source.index("pub fn discover_workspace_at(root: &Path) -> WorkspaceSnapshot {")
end = source.index("fn build_snapshot(", start)
replacement = r'''pub fn discover_workspace_at(root: &Path) -> WorkspaceSnapshot {
...
print(f"workspace.rs now contains {len(path.read_text().splitlines())} lines")
