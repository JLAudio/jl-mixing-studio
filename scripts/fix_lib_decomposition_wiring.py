...
# lib_tests.rs previously inherited std::fs and std::path::Path through
# `use super::*` from lib.rs. The production imports disappear in this
# decomposition, so make test-only dependencies explicit instead of retaining
# unused production-level imports.
lib_tests = SRC / "lib_tests.rs"
lib_tests_text = lib_tests.read_text()
for required_import in ["use std::fs;\n", "use std::path::Path;\n"]:
    if required_import not in lib_tests_text:
        lib_tests_text = required_import + lib_tests_text
