...
- JL Mixing Automation v1.3.1 installed to its default `~/.local/bin` command location or available on `PATH` to exercise automation workflows.
...
For the current functional baseline, the installed `VERSION` file must report `1.3.1`, and `new-client --help`, `new-mix --help`, `validate-intake --help`, `new-revision --help`, `approve-mix --help`, and `create-delivery --help` must succeed. JL Mixing Automation v1.3.1 installs individual workflow commands; it does not provide a top-level `jl-mixing` command. On Windows, where JL Mixing Automation v1.3.1 is not supported, the application reports guided automation as unavailable without preventing supported report reading and workspace browsing.
...
Delivery creation resolves the same exact project identity and uses only allowlisted combinations of `--dry-run`, `--zip`, and `--overwrite`. Default mode is limited to a project without a package; overwrite requires a correlated existing package and preserves the exact Delivery Notes bytes. The frontend verifies the preview mode and ZIP choice, and Rust re-discovers the workspace after success to reconcile the delivered pointer, manifest revision, file paths, classifications, notes, and requested fixed-name ZIP while project history remains unchanged. Studio exposes no project path, include/exclude patterns, working-prefix override, or automatic escalation from a rejected overwrite to clean replacement.

Destructive replacement adds only the released `--clean` flag. The dry-run deletion list must contain unique bounded safe relative paths and is shown in full before the user types `CLEAN <project-id>`. Rust re-enumerates the fixed delivery directory immediately before execution and rejects a changed inventory. Post-operation checks require the normal manifest reconciliation, recreated regular Delivery Notes, the requested ZIP, and removal of every previewed path not intentionally recreated. An uncertain result is never retried automatically.
