...
Studio may add only the released `--zip` and `--overwrite` flags to the fixed `create-delivery` command. Every request runs `--dry-run` first and verifies the reported replacement mode, ZIP choice, project identity, revisions, method, and selected path set. Default mode is allowed only without an existing package; overwrite is allowed only with a correlated delivery manifest and delivered pointer. A successful overwrite must preserve the pre-existing Delivery Notes bytes, keep project history unchanged, reconcile the new manifest and delivered pointer, and parse Automation's exact `ZIP:` result. The reported name must match `<project-id>-rev-<approved-NN>-<14-digit local timestamp>.zip`; Studio rejects the legacy fixed filename, a mismatched revision, and a missing or symlinked reported archive. Historical ZIPs are not selected by directory order or modification time. Changed path sets remain an Automation rejection rather than silently escalating to clean replacement. This scope is tracked in [Issue #36](https://github.com/JLAudio/jl-mixing-studio/issues/36) and hardened by [Issue #45](https://github.com/JLAudio/jl-mixing-studio/issues/45).
